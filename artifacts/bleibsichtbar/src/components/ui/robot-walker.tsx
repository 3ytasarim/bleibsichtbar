// Vendored & adapted from the 21st.dev registry (alexperezcedeno/robot-hero):
// the original is a full-page hero (3D robot + text watermark + nav bar) that
// follows the mouse cursor. This keeps just the robot mesh itself, swapped
// from mouse-tracking to an autonomous side-to-side "wander" (time-based
// oscillation) so it works as small background motion inside a card rather
// than needing a whole hero viewport and a pointer to react to.
// @ts-nocheck
...
function RobotPrototype({
  ...
  const config = {
    ...
    headRotSpeed: 20.0,
    bodyTiltX: 0.0,
    // Toned down from the source hero's values (0.95 / 1.8): those were
    // tuned for a fullscreen camera. In this small card-sized canvas the
    // same swing carries the model far enough off-axis that perspective
    // makes it look like it grows/warps near the pointer's edges.
    bodyTiltY: 0.45,
    headLookX: 0.3,
    headLookY: 0.9,
  };
  ...
  useFrame((state, delta) => {
    ...
    // Original behavior from the source component: the robot's body/head
    // follow the pointer within this canvas's own bounds. Clamped to a
    // narrow band (not the full [-1,1] the pointer can report) — the model
    // never has to travel/turn far, so it stays put near the frame's edges
    // instead of swinging out to where perspective makes it look zoomed.
    const tx = THREE.MathUtils.clamp(state.pointer.x, -0.4, 0.4);
    const ty = THREE.MathUtils.clamp(state.pointer.y, -0.4, 0.4);

    const maxMoveX = state.viewport.width / 6;
...
export interface RobotWalkerProps {
  className?: string;
  scale?: number;
  color?: string;
  pantallaColor?: string;
...
/**
 * Just the robot — body/head follow the cursor while it's over this canvas
 * (tracked within the canvas's own bounds — a larger custom eventSource was
 * tried and reverted: r3f's pointer normalization can exceed the expected
 * [-1,1] range once the tracked area is bigger than the canvas itself,
 * sending the model's position/rotation to extreme values), transparent
 * background, meant to sit as a layer inside a card. `prefers-reduced-motion`
 * freezes the render loop (no idle animation) rather than disabling the
 * pointer-follow.
 */
export function RobotWalker({ className, scale = 1, color = "#2563eb", pantallaColor = "#93c5fd" }: RobotWalkerProps) {
  ...
  // r3f's own ResizeObserver can measure this Canvas's container too early
  // when it mounts lazily inside a grid layout (before the grid has settled
  // to its final track sizes), leaving the canvas stuck at the browser's
  // 300×150 default. A nudge shortly after mount makes it re-measure.
  useEffect(() => {
  ...
  return (
    <div className={cn("absolute inset-0", className)} aria-hidden>
      ...
      >
        ...
        <directionalLight position={[2, 4, 3]} intensity={0.45} color="#ffffff" />
        <directionalLight position={[-3, 1, -2]} intensity={0.25} color="#bfdbfe" />
        <ResponsiveGroup scale={scale}>
          <RobotPrototype color={color} pantallaColor={pantallaColor} pantallaBrillo={1.1} metalness={0} />
        </ResponsiveGroup>
      </Canvas>
    </div>
  );
