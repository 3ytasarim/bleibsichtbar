...
export type AnimatedCountdownProps = {
  ...
  /** Addition: when set, counts UP live from this past date instead of down to targetDate. */
  elapsedFrom?: Date | string | number;
  ...
  size?: CountdownSize;
  ariaLabel?: string;
  /** Addition: override the built-in English unit labels (e.g. for i18n). */
  unitLabels?: Partial<Record<CountdownUnitKey, string>>;
};
...
// Addition (not part of the original component): the original only ever
// counts DOWN to a future targetDate. `elapsedFrom` counts UP, live, from a
// past date — same tick/animation machinery, opposite time source.
function getElapsed(elapsedFrom?: AnimatedCountdownProps["elapsedFrom"]): TimeLeft {
  const from = toDateTime(elapsedFrom);
  if (!from) return DEFAULT_STATIC_TIME;

  const total = Math.max(0, Date.now() - from);
  const secondsTotal = Math.floor(total / 1000);
  const days = Math.floor(secondsTotal / 86400);
  const hours = Math.floor((secondsTotal % 86400) / 3600);
  const minutes = Math.floor((secondsTotal % 3600) / 60);
...
export function AnimatedCountdown({
  ...
  const labels = { ...UNIT_LABELS, ...unitLabels };
  const reduceMotion = useReducedMotion() === true;
  const isElapsedMode = !!elapsedFrom;
  const isStatic = staticMode ?? (!targetDate && !isElapsedMode);
  ...
  const completed = mounted && !isStatic && !isElapsedMode && isFinished(displayTimeLeft);

  React.useEffect(() => {
    setMounted(true);
    if (isStatic) return;

    const tick = () => setTimeLeft(isElapsedMode ? getElapsed(elapsedFrom) : getTimeLeft(targetDate));
    tick();
    const interval = window.setInterval(tick, 1000);

    return () => window.clearInterval(interval);
  }, [isStatic, targetDate, isElapsedMode, elapsedFrom]);
