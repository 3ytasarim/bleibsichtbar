...
import { CheckCircle2, Brain, Target, Database, BarChart3, Clock } from "lucide-react";

/* ─── Galaxy background (same as Onboarding.tsx) ─── */
function GalaxyCanvas() {
  ...
  useEffect(() => {
    ...
    const shootingStarBaseLength=12,shootingStarBaseLifespan=90;
    const shootingStarsColors=["#a1ffba","#a1d2ff","#fffaa1","#ffa1a1"];
    const mwStarCount=80000,mwRandomStarProp=0.2,mwClusterCount=300,mwClusterStarCount=1500;
    const mwClusterSize=120,mwClusterSizeR=80,mwClusterLayers=10,mwAngle=0.6;
    const mwHueMin=150,mwHueMax=300,mwWhiteProportionMin=50,mwWhiteProportionMax=65;
    ...
    class ShootingStar {
      ...
      update(){this.framesLeft--;const len=shootingStarBaseLength*(this.framesLeft/shootingStarBaseLifespan);ctx.beginPath();ctx.moveTo(this.x,this.y);ctx.lineTo(this.x-this.speedX*len/4,this.y-this.speedY*len/4);ctx.strokeStyle=this.color;ctx.lineWidth=0.8;ctx.globalAlpha=this.framesLeft/shootingStarBaseLifespan;ctx.stroke();ctx.globalAlpha=1;this.x+=this.speedX;this.y+=this.speedY;}
    }
    ...
    ctxMw.save();ctxMw.translate(mwCX,mwCY);ctxMw.rotate(mwAngle);
    const whiteProp=(mwWhiteProportionMin+Math.random()*(mwWhiteProportionMax-mwWhiteProportionMin))/100;
...
export default function OnboardingAI() {
    ...
    try {
      await fetch("/api/onboarding", {
        ...
        body: JSON.stringify({
          ...
          data: { ...f, formType: "KI-Automatisierungs Bedarfsanalyse" },
        }),
      });
    } catch (_) {}
  ...
  const bereicheOptions = [
    "KI-Telefonassistent",
    "KI-Chatbot für Webseite",
    "WhatsApp-Automatisierung",
    "Terminbuchung / Kalender-Automatisierung",
    "Sonstiges",
  ];

  const aufgabenOptions = [
  ...
  const crmOptions = ["Ja, und zwar:", "Nein", "Noch nicht, aber geplant"];

  const anfragenOptions = ["0–20", "20–50", "50–100", "100+"];

  const umsetzungOptions = [
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-4 sm:px-6 py-16 sm:py-24">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="text-center mb-14">
          <div className="inline-flex items-center gap-2 bg-orange-500/15 border border-orange-500/30 rounded-full px-5 py-2 mb-8">
            ...
            <span className="text-orange-400 text-sm font-semibold tracking-wide">🤖 BleibSichtbar – KI Bedarfsanalyse</span>
          </div>
          <h1 className="text-4xl sm:text-5xl font-bold text-white mb-5 leading-tight">
            KI-Automatisierungs<br /><span className="text-orange-400">Bedarfsanalyse</span>
          </h1>
          <div className="inline-block rounded-2xl px-6 py-3" style={{ background: "rgba(6,13,31,0.65)", backdropFilter: "blur(8px)" }}>
            <p className="text-white/80 text-base max-w-xl mx-auto leading-relaxed">
              In wenigen Minuten finden wir gemeinsam heraus, welche KI-Lösung am besten zu Ihrem Unternehmen passt.
            </p>
          </div>
        </motion.div>
        ...
        <AnimatePresence mode="wait">
          {!submitted ? (
            <motion.form key="form" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
              ...
              {/* Bereich */}
              <Section icon={Brain} color="bg-purple-500/20 text-purple-300" title="Bereich" subtitle="Womit kann KI bei Ihnen helfen?">
                <div>
                  <QLabel n={1} text="Für welchen Bereich interessieren Sie sich?" required />
                  <OptionGrid options={bereicheOptions} selected={f.bereiche} onChange={v => toggleMulti("bereiche", v)} multi />
                  {f.bereiche.includes("Sonstiges") && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="mt-3 overflow-hidden">
                      <TextInput value={f.bereicheSonstiges} onChange={v => set("bereicheSonstiges", v)} placeholder="Bitte beschreiben ..." />
                    </motion.div>
                  )}
                </div>
              </Section>
              ...
              {/* Aufgaben */}
              <Section icon={Target} color="bg-orange-500/20 text-orange-300" title="Aufgaben" subtitle="Was soll die KI hauptsächlich tun?">
                <div>
                  <QLabel n={2} text="Welche Aufgabe soll die KI hauptsächlich übernehmen?" required />
                  <OptionGrid options={aufgabenOptions} selected={f.aufgaben} onChange={v => toggleMulti("aufgaben", v)} multi />
                </div>
              </Section>

              {/* CRM */}
              <Section icon={Database} color="bg-blue-500/20 text-blue-300" title="System" subtitle="Bestehende Tools & Software">
                <div>
                  <QLabel n={3} text="Nutzen Sie bereits ein Termin- oder CRM-System?" required />
                  <OptionGrid
                    options={crmOptions}
                    selected={f.crm}
                    onChange={v => { set("crm", v === f.crm ? "" : v); if (v !== "Ja, und zwar:") set("crmDetail", ""); }}
                  />
                  {f.crm === "Ja, und zwar:" && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="mt-3 overflow-hidden">
                      <TextInput value={f.crmDetail} onChange={v => set("crmDetail", v)} placeholder="z. B. Calendly, HubSpot, Salesforce ..." />
                    </motion.div>
                  )}
                </div>
              </Section>
              ...
              {/* Anfragen / Woche */}
              <Section icon={BarChart3} color="bg-green-500/20 text-green-300" title="Volumen" subtitle="Aktuelle Anfragelast">
                <div>
                  <QLabel n={4} text="Wie viele Kundenanfragen erhalten Sie ungefähr pro Woche?" required />
                  <OptionGrid options={anfragenOptions} selected={f.anfragen} onChange={v => set("anfragen", v === f.anfragen ? "" : v)} />
                </div>
              </Section>

              {/* Umsetzung */}
              <Section icon={Clock} color="bg-cyan-500/20 text-cyan-300" title="Zeitplan" subtitle="Wann soll es losgehen?">
                <div>
                  <QLabel n={5} text="Wie schnell möchten Sie die Lösung umsetzen?" required />
                  <OptionGrid options={umsetzungOptions} selected={f.umsetzung} onChange={v => set("umsetzung", v === f.umsetzung ? "" : v)} />
                </div>
              </Section>
