...
function makeServices(t: Translations) {
  ...
  return [
    { icon: <Edit3 className="w-6 h-6" />, title: sm.service1Title, desc: sm.service1Desc },
    { icon: <Camera className="w-6 h-6" />, title: sm.service2Title, desc: sm.service2Desc },
    { icon: <Clock className="w-6 h-6" />, title: sm.service3Title, desc: sm.service3Desc },
    { icon: <Camera className="w-6 h-6" />, title: sm.service4Title, desc: sm.service4Desc },
    { icon: <MessageSquare className="w-6 h-6" />, title: sm.service5Title, desc: sm.service5Desc },
    { icon: <BarChart3 className="w-6 h-6" />, title: sm.service6Title, desc: sm.service6Desc },
  ];
}
...
export default function SocialMedia() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!form.company.trim()) {
      setFormError(sm.formCompanyRequired);
      return;
    }
    setFormError("");
    setSubmitted(true);
    ...
    <PublicLayout>
      ...
      <section className="relative bg-primary text-white overflow-hidden pt-32 pb-24">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          ...
          <motion.h1 custom={1} variants={heroFadeUp} initial="hidden" animate="visible"
            ...
            {sm.heroTitle1}<br />
            <span className="text-accent">{sm.heroTitle2}</span>
          </motion.h1>
          ...
          <motion.div custom={3} variants={heroFadeUp} initial="hidden" animate="visible"
            ...
            <Button asChild size="lg" className="rounded-full px-8 bg-accent hover:bg-accent/90 text-white font-bold">
              <a href="#analysebogen">{sm.heroCta1}</a>
            </Button>
            <Button asChild size="lg" variant="outline" className="rounded-full px-8 border-white/30 text-white bg-transparent hover:bg-white/10">
              <Link href="/kontakt">{sm.heroCta2}</Link>
            </Button>
          </motion.div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}>
            <motion.div variants={fadeUp} className="text-center mb-16">
              <p className="text-accent font-semibold text-sm tracking-widest uppercase mb-3">{sm.servicesLabel}</p>
              <h2 className="text-4xl md:text-5xl font-display font-bold mb-4">{sm.servicesTitle} <span className="text-accent">{sm.servicesTitleAccent}</span></h2>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}
            ...
            <motion.h2 variants={fadeUp} className="text-4xl md:text-6xl font-display font-bold text-white">
              {sm.processTitle}{" "}
              <span className="text-accent">{sm.processTitleAccent}</span>
            </motion.h2>
      ...
      <section className="py-16 bg-primary text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <p className="text-white/60 text-sm font-medium tracking-widest uppercase mb-6">{sm.platformsLabel}</p>
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
              <div>
                ...
                <motion.h2 variants={fadeUp} className="text-4xl md:text-6xl font-display font-black leading-tight" style={{ color: "#0a1628" }}>
                  {sm.casesTitle} <span className="text-accent">{sm.casesTitleAccent}</span>
                </motion.h2>
              </div>
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}>
            <motion.div variants={fadeUp} className="text-center mb-12">
              <p className="text-accent font-semibold text-sm tracking-widest uppercase mb-3">{sm.formLabel}</p>
              <h2 className="text-4xl md:text-5xl font-display font-bold mb-4">{sm.formTitle} <span className="text-accent">{sm.formTitleAccent}</span></h2>
            ...
            {submitted ? (
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-16 bg-green-50 rounded-3xl border border-green-100">
                ...
                <h3 className="text-2xl font-display font-bold mb-2">{sm.formThanksTitle}</h3>
                <p className="text-muted-foreground">{sm.formThanksDesc}</p>
              </motion.div>
            ) : (
              <motion.form noValidate variants={fadeUp} onSubmit={handleSubmit} className="bg-gray-50 rounded-3xl p-8 md:p-12 border border-gray-100 space-y-8">
                ...
                <div>
                  <label className="block text-sm font-bold mb-2">{sm.formQ1}</label>
                  <input value={form.company} onChange={e => setForm(f => ({...f, company: e.target.value}))}
                    ...
                    placeholder={sm.formQ1Placeholder} />
                </div>

                <div>
                  <label className="block text-sm font-bold mb-2">{sm.formQ2} <span className="font-normal text-muted-foreground">{sm.formQ2Sub}</span></label>
                  <input value={form.contact} onChange={e => setForm(f => ({...f, contact: e.target.value}))}
                    className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent bg-white"
                    placeholder={sm.formQ2Placeholder} />
                </div>

                <div>
                  <label className="block text-sm font-bold mb-3">{sm.formQ3}</label>
                ...
                <div className="grid grid-cols-3 gap-4">
                  {[
                    { key: "feedposts", label: sm.formQ4a },
                    { key: "reels", label: sm.formQ4b },
                    { key: "stories", label: sm.formQ4c },
                  ].map(f => (
                    <div key={f.key}>
                      ...
                      <select value={(form as any)[f.key]} onChange={e => setForm(prev => ({...prev, [f.key]: e.target.value}))}
                        ...
                        <option value="">{sm.formSelectDefault}</option>
                ...
                <div>
                  <label className="block text-sm font-bold mb-3">{sm.formQ5}</label>
                  <div className="flex flex-wrap gap-3">
                    {[sm.formQ5o1, sm.formQ5o2, sm.formQ5o3].map(opt => (
                ...
                {[
                  { key: "hasWebsite", label: sm.formQ6 },
                  { key: "ads", label: sm.formQ7 },
                  { key: "wishes", label: sm.formQ8 },
                  { key: "goals", label: sm.formQ9 },
                  { key: "previousAgency", label: sm.formQ10 },
                  { key: "priorities", label: sm.formQ11 },
                  { key: "dislikes", label: sm.formQ12 },
                  { key: "collaboration", label: sm.formQ13 },
                ].map(field => (
                  <div key={field.key}>
                    ...
                    <textarea value={(form as any)[field.key]} onChange={e => setForm(prev => ({...prev, [field.key]: e.target.value}))}
                      ...
                      placeholder={sm.formPlaceholder} />
                  </div>
                ))}
                ...
                <p className="text-center text-xs text-muted-foreground">{sm.formFootnote}</p>
              </motion.form>
            )}
          </motion.div>
        </div>
