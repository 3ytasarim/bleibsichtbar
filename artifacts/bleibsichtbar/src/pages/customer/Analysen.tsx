...
import { useGetPortalFiles } from "@workspace/api-client-react";
import { FolderOpen, Loader2, ExternalLink } from "lucide-react";

export default function CustomerAnalysen() {
  const { data, isLoading, isError } = useGetPortalFiles();

  return (
    <CustomerPortalLayout>
      <div className="flex flex-col h-[calc(100vh-8rem)]">
        <h1 className="text-2xl sm:text-3xl font-bold font-display flex items-center gap-3 shrink-0">
          <FolderOpen className="w-7 h-7 text-[#2563eb]" /> Dateien
        </h1>
        <p className="text-muted-foreground mt-2 shrink-0">Ihr Content-Archiv — Berichte, Bilder und Dokumente.</p>

        <div className="mt-6 flex-1 min-h-0 bg-card rounded-2xl border border-border overflow-hidden flex flex-col">
          {isLoading ? (
            <div className="flex-1 flex items-center justify-center text-muted-foreground gap-2">
              <Loader2 className="w-5 h-5 animate-spin" /> Lade Dateien...
            </div>
          ) : isError || !data?.enabled || !data?.shareLink ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center px-6">
              <FolderOpen className="w-10 h-10 mx-auto text-muted-foreground/40 mb-3" />
              <p className="font-medium">Dateispeicher ist für dieses Konto noch nicht aktiviert.</p>
              <p className="text-sm text-muted-foreground mt-1">
                Bitte wenden Sie sich an Ihren Ansprechpartner, um den Dateibereich einzurichten.
              </p>
            </div>
          ) : (
            <>
